...
class ConfigValidator:
    ...
    def _load_yaml_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationValidationError(f"Invalid YAML syntax in {file_path}: {e}")
        except Exception as e:
            raise ConfigurationValidationError(f"Failed to load {file_path}: {e}")
    
    def _format_validation_errors(self, errors: List[jsonschema.ValidationError]) -> List[str]:
        """
        ...
        for error in errors:
            ...
            if error.validator == "required":
                ...
                message = f"Value '{error.instance}' at '{path}' must be one of: {allowed}"
    ...
    def validate_site_config(self, site_id: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if errors:
                ...
                logger.warning(f"Site configuration validation failed for {site_id}: {len(errors)} errors")
            ...
            if custom_errors:
                logger.warning(f"Site configuration custom validation failed for {site_id}: {len(custom_errors)} errors")
    ...
    def _validate_site_config_custom(self, config_data: Dict[str, Any]) -> List[str]:
        ...
        for client in config_data.get('protocol_clients', []):
            ...
            if invalid_modules:
                ...
                errors.append(f"Protocol client '{client_id}' references non-existent modules: {list(invalid_modules)}")
        
        # Validate control strategy references
        for strategy_name, strategy in config_data.get('control_strategies', {}).items():
            for param in strategy.get('inputs', []) + strategy.get('outputs', []):
                if '.' in param:
                    module_name = param.split('.')[0]
                    if module_name not in modules:
                        errors.append(f"Control strategy '{strategy_name}' references non-existent module '{module_name}'")
        ...
        for category, alarms in config_data.get('alarm_definitions', {}).items():
            for alarm_name, alarm in alarms.items():
                ...
                if '.' in param:
                    module_name = param.split('.')[0]
                    if module_name not in modules:
                        errors.append(f"Alarm '{category}.{alarm_name}' references non-existent module '{module_name}'")
    ...
    def validate_module_templates(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if errors:
                error_messages = self._format_validation_errors(errors)
                logger.warning(f"Module templates validation failed: {len(errors)} errors")
                return False, error_messages
    ...
    def validate_parameter_specifications(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if errors:
                error_messages = self._format_validation_errors(errors)
                logger.warning(f"Parameter specifications validation failed: {len(errors)} errors")
                return False, error_messages
    ...
    def validate_all_configurations(self, site_ids: Optional[List[str]] = None) -> Dict[str, Tuple[bool, List[str]]]:
        """
    ...
    def validate_configuration_compatibility(self, site_id: str) -> Tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            available_templates = set(module_templates.get('module_templates', {}).keys())
            ...
            for module in site_modules:
                ...
                if base_module not in available_templates:
                    errors.append(f"Site references undefined module template: '{base_module}' (from instance '{module}')")
            
            # Validate parameter references in module templates
            available_params = set(param_specs.get('parameter_specifications', {}).keys())
            
            for template_name, template in module_templates.get('module_templates', {}).items():
                for sensor_list in ['required_sensors', 'optional_sensors', 'actuators']:
                    for param in template.get(sensor_list, []):
                        if param not in available_params:
                            errors.append(f"Module template '{template_name}' references undefined parameter: '{param}'")
...
def validate_configurations_cli(config_path: str = None, site_ids: List[str] = None) -> int:
    """
