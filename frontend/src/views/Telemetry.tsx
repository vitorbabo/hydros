...
export function Telemetry() {
  ...
  }, [latest, selectedAssetGroup, assetGroups])
  ...
  const currentObservation = selectedSensor ? latest[selectedSensor] : null

  // Determine status based on measurement type and value
  const getStatusFromValue = (measurement: string, value: number, unit: string): 'normal' | 'warning' | 'alarm' => {
    // Apply scaling factor correction (values appear to be * 100)
    ...
    switch (measurement) {
      ...
      case 'run_status':
        return 'normal' // Boolean values are generally normal unless there's specific logic
      default:
        return 'normal'
    }
  }
  ...
  // Get available asset groups that have data
  const availableGroups = useMemo(() => {
    return Object.keys(assetGroupConfig).filter(groupId => {
      if (groupId === 'all') return Object.keys(latest).length > 0
    ...
    return (
      <div className="p-6 space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Sensor Telemetry</h1>
        ...
        <div className="bg-white rounded-lg shadow-md border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">
            Waiting for sensor observations from the Hydros system...
          </p>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Sensor Telemetry</h1>
          <p className="mt-1 text-sm text-gray-500">
            {Object.keys(latest).length} sensors • {availableAssets.length} assets • Real-time monitoring
          </p>
        </div>
        <div className="flex items-center gap-2">
          <StatusIndicator status={connectionStatus} showLabel />
        ...
        <div className="w-80 space-y-4">
          ...
          <div className="bg-white rounded-lg shadow-md border border-gray-200 p-4">
            ...
            <div className="space-y-2">
              {availableGroups.map((groupId) => {
                const group = assetGroupConfig[groupId]
                const Icon = group.icon
                const count = groupId === 'all' 
                  ? availableAssets.length 
                  : (assetGroups[groupId] || []).length
          ...
          {currentObservation && (
            <MetricCard
              ...
              status={getStatusFromValue(currentObservation.measurement, currentObservation.value, currentObservation.unit) === 'normal' ? 'normal' : 'warning'}
              icon={React.createElement(measurementIcons[currentObservation.measurement] || Gauge, { className: "w-6 h-6" })}
            />
          )}
        </div>
              ...
              <div>
                ...
                {currentObservation && (
                  <p className="text-sm text-gray-500">
                    {currentObservation.component_type} • Quality: {currentObservation.quality} • Tag: {currentObservation.raw_tag}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-4">
                {currentObservation && getStatusFromValue(currentObservation.measurement, currentObservation.value, currentObservation.unit) !== 'normal' && (
                  <div className="flex items-center gap-2 text-yellow-600">
                    <AlertTriangle className="w-4 h-4" />
                    <span className="text-sm font-medium">Warning</span>
                  </div>
                )}
                <button className="industrial-button text-sm">
                  Export Data
                </button>
              </div>
            </div>
              ...
              {chartData.length > 0 ? (
                  ...
                  <LineChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Tooltip 
                      ...
                      formatter={(value: number) => {
                        const displayVal = currentObservation?.unit === 'bool' 
                          ? (value > 0 ? 'ON' : 'OFF')
                          : `${(value / 100).toFixed(2)} ${currentObservation?.unit || ''}`
                        return [
                          displayVal,
                          currentObservation?.measurement.replace(/_/g, ' ') || ''
                        ]
                      }}
                    />
                    <Line 
                ...
                <div className="h-full flex items-center justify-center text-gray-500">
                  <div className="text-center">
                    ...
                    <p className="text-sm mt-2">Historical data will appear as observations are received</p>
                  </div>
                </div>
              )}
            </div>
