...
from .data_mapper import DataQuality, PLCConnection, PLCDataReader


class ModbusTcpReader(PLCDataReader):
    """Async Modbus TCP client implementation"""

    def __init__(self, connection: PLCConnection):
        super().__init__(connection)
        self.async_client = None  # Async client only
    ...
    def connect(self) -> bool:
        """Sync connect not supported - use connect_async() instead"""
        raise NotImplementedError(
            "Sync connect not supported - use connect_async() instead"
        )

    def read_tag(self, tag_address: str, data_type: str) -> Tuple[Any, DataQuality]:
        """Sync read not supported - use read_tag_async() instead"""
        raise NotImplementedError(
            "Sync read not supported - use read_tag_async() instead"
        )

    def read_tags(self, tags: List[str]) -> Dict[str, Tuple[Any, DataQuality]]:
        """Sync read not supported - use async methods instead"""
        raise NotImplementedError("Sync read not supported - use async methods instead")

    async def connect_async(self) -> bool:
        """Connect to Modbus TCP server (async client)"""
        try:
            self.async_client = ModbusClient.AsyncModbusTcpClient(
            ...
            self.is_connected = self.async_client.connected

            if self.is_connected:
                self.logger.info(
                    f"Connected to Modbus TCP (async) at {self.connection.ip_address}:{self.connection.port}"
                )
            else:
                self.logger.error("Failed to connect to Modbus TCP server (async)")

            return self.is_connected

        except Exception as e:
            self.last_error = str(e)
            self.logger.error(f"Modbus TCP async connection failed: {e}")
            return False
    ...
    async def read_tag_async(
        ...
        """Read single Modbus register (async)"""
        if not self.async_client or not self.async_client.connected:
            return None, DataQuality.COMM_ERROR

        try:
            register_type, address = self._parse_modbus_address(tag_address)

            # Read based on register type with device_id as keyword argument
            device_id = getattr(
                self.connection, "unit_id", getattr(self.connection, "device_id", 1)
            )
            if register_type == "coils":
                result = await self.async_client.read_coils(
                    address, count=1, device_id=device_id
                )
            elif register_type == "discrete_inputs":
                result = await self.async_client.read_discrete_inputs(
                    address, count=1, device_id=device_id
                )
            elif register_type == "input_registers":
                result = await self.async_client.read_input_registers(
                    address, count=1, device_id=device_id
                )
            elif register_type == "holding_registers":
                result = await self.async_client.read_holding_registers(
                    address, count=1, device_id=device_id
                )
            else:
...
class OPCUAReader(PLCDataReader):
    ...
    def __init__(self, connection: PLCConnection):
        ...
        if not OPCUA_AVAILABLE:
            raise ImportError(
                "opcua library is required. Install with: pip install opcua"
            )
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if (
                hasattr(self.connection, "endpoint_url")
                and self.connection.endpoint_url
            ):
    ...
    def read_tag(self, tag_address: str, data_type: str) -> Tuple[Any, DataQuality]:
        ...
        try:
            ...
            self.logger.error(
                f"Unexpected error reading OPC UA node {tag_address}: {e}"
            )
            return None, DataQuality.BAD
...
class S7Reader(PLCDataReader):
    ...
    def __init__(self, connection: PLCConnection):
        ...
        if not S7_AVAILABLE:
            raise ImportError(
                "snap7 library is required. Install with: pip install snap7"
            )
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.is_connected = True
            self.logger.info(
                f"Connected to S7 PLC at {self.connection.ip_address} (rack {rack}, slot {slot})"
            )
            return True
...
def create_plc_reader(connection: PLCConnection) -> PLCDataReader:
    ...
    if "modbus" in protocol_name:
        ...
        # Fallback - no mock reader available
